...
export default function UserInfoCard() {
  ...
  const handleSave = async () => {
    try {
      ...
      // Prepare update request - use formData values, or fall back to currentUser values
      // Always send actual values, not null
      const updateRequest: UpdateProfileRequest = {
        userName: fullName !== "" ? fullName : (currentUser?.name || null),
        email: formData.email.trim() !== "" ? formData.email.trim() : (currentUser?.email || null),
        country: currentUser?.country || null,
        mobile: formData.mobile.trim() !== "" ? formData.mobile.trim() : (currentUser?.mobile || null),
        transactionPassword: null,
        userNodeId: currentUser?.nodeId || null,
      };
      
      // Ensure we're not sending all nulls - validate that at least one field has a value
      if (!updateRequest.userName && !updateRequest.email && !updateRequest.mobile) {
        console.error("All fields are null! This should not happen.");
